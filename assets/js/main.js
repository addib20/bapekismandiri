function validateMaskedValue(val,bul,y){alert("masked")}
    function validateValue(val,bul,y){alert("validate")}
$(function() {
	...
	var n = d.getMonth(); 
	var y = d.getFullYear(); 
	var bul=["I","II","III","IV","V","VI","VII","VIII","IX","X","XI","XII"];
	
	var no_memo_lpj = $('#no_memo_lpj').val();
	if(no_memo_lpj == ""){
		$('#no_memo_lpj').inputmask("M-999/AAA-AAA/"+bul[n]+"/"+y);
	}
	
	var no_memo = $('#no_memo').val();
	if(no_memo == ""){
		$('#no_memo').inputmask({ "mask": "M-999/AAA-AAA/"+bul[n]+"/"+y});
	}
	var no_memo_pengajuan = $('#no_memo_pengajuan').val();
	if(no_memo_pengajuan == ""){
		$('#no_memo_pengajuan').inputmask({ "mask": "M-999/AAA-AAA/"+bul[n]+"/"+y});
	}
	$('#nik').inputmask("9999.99.99");
	$('#inisial').inputmask("AAA");
	
	$("#no_memo").change(function(){
		var no_memo=$(this).val();
		var dataString = 'no_memo='+ no_memo;
		$.ajax({
			type: "POST",
			url: "/simpel/pelatihan/getPengajuan",
			data: dataString,
			dataType: 'json',
			cache: false,
			success: function(res){
				$("#nama_kegiatan").val(res.nama_pelatihan);
			}
		});
	});
	
	$("#no_memo_pengajuan").change(function(){
		var no_memo=$(this).val();
		var dataString = 'no_memo='+ no_memo;
		$.ajax({
			type: "POST",
			...
			dataType: 'json',
			  success: function(res){
				$(".loading_ajax").hide();
				if(res.nama_pelatihan!=""){
					alert("No. Memo sudah digunakan");
					$('#no_memo_pengajuan').val("");
				}
			  },
			  beforeSend:function(){
				$(".loading_ajax").show();
					...
					success: function(doc) {
						...
						$(doc).each(function() {
							events.push({
								...
								className: $(this).attr('className')// will be parsed
							});
						});
						callback(events);
					}
...
function formWysiwyg() {
    ...
    /*----------- BEGIN cleditor CODE -------------------------*/
    editor = $("textarea").cleditor({width: "600px", height: "300px"})[0].focus();
    $(window).resize();
...
function table_ppl() {
	...
	var uri = url.slice(Math.max(url.length - 1, 1));
    /*----------- BEGIN TABLESORTER CODE -------------------------*/
    /* required jquery.tablesorter.min.js*/
    $(".sortableTable").tablesorter();
    /*----------- END TABLESORTER CODE -------------------------*/
