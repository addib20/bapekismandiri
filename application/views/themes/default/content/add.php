                        	...
                        	<?php
                        		...
                        		if(!isset($field['type']) || $field['type']=='' || $field['type']=='input') {
									if(!isset($field['readonly'])){
										echo form_input(array('id'=>$field['name'],'name'=>$field['name'],'class'=>$class_validate.' form-control input-xxlarge','value'=>set_value($field['name'])));
									}
									else{
								...
								elseif($field['type']=='number'){
									echo form_input(array('id'=>$field['name'],'name'=>$field['name'],'style'=>'text-align: right','onKeyPress'=>'return isNumberKey(event)','class'=>$class_validate.' form-control input-xxlarge','value'=>set_value($field['name'])));
								}
								elseif($field['type']=='select') {
									echo form_dropdown($field['name'],$opt[$field['opt']],'default','class="'.$class_validate.' " id="'.$field['name'].'"');
								}
								elseif($field['type']=='multiple') {
									echo '<select multiple class="forn-control '.$class_validate.'" id= "'.$field['name'].'" name= "'.$field['name'].'[]">';
								...
								elseif($field['type']=='radio'){
									foreach($radio[$field['radio']] as $o){
										...
										echo "<label class='label-align'>".$o['label']."</label>";
										echo "</label>";
									}
								}
								elseif($field['type']=='textarea') {
									echo form_textarea(array('id'=>$field['name'],'name'=>$field['name'],'class'=>$class_validate.' form-control','value'=>set_value($field['name'])));
								}
								...
								elseif($field['type']=='upload') {
									echo form_upload(array('id'=>$field['name'],'name'=>$field['name'],'class'=>$class_validate));
								}
								...
								elseif($field['type']=='datepicker'){
									...
									echo form_input(array('id'=>$field['name'],'readonly'=>true,'name'=>$field['name'],'class'=>$class_validate.' form-control input-xxlarge','value'=>set_value($field['name'],date('d-m-Y'))));
